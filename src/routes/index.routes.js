...
router.get(url + "/data", (req, res, next) => {
  res.json({
    status: 200,
    message: "OK",
    body: [
    ...
    kernel: "(GNU/Linux 6.8.0-48-generic x86_64)",
      },
      {
        docker_version: "Docker version 24.0.7, build 24.0.7-0ubuntu4.1",
        docker_compose_version: "Docker Compose version v2.30.3",
        github_actions: "actions/checkout@v4"
      ...
      {
        update:"credentials udpate"
      },
      {
        password: "Encrypted password and create routes"
      }
    ]
  });
});
